...
`widthMeters` задаётся в мировых метрах и сохраняет существующее имя JSON-поля проекта.
...
геометрия пути масштабируется, но `widthMeters` по умолчанию не масштабируется.
...
Dot должен иметь визуальный диаметр, связанный с `widthMeters`.
...
Bounds marking должны учитывать половину `widthMeters`.
