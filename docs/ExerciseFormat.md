...
`markings` описывает дополнительную цветную разметку элемента.
...
Для первой версии Exercise Editor markings можно не реализовывать.

Когда поддержка появится, первоначально достаточно:
...
Координаты точек масштабируются вместе с ExerciseInstance.

`widthMeters` является физическим размером и не масштабируется.
