...
## Strict parser for the self-contained Exported Track v4 runtime contract.
...
static func _validate_markings(markings: Array, source_name: String) -> String:
	for index in markings.size():
		...
		if not marking is Dictionary or not _non_empty_string(marking.get("id")):
			return _contract_error(source_name, "%s.id must be non-empty" % prefix)
		if not _non_empty_string(marking.get("type")):
			return _contract_error(source_name, "%s.type must be non-empty" % prefix)
		var points: Variant = marking.get("points")
		...
		if marking["type"] == "line" and points.size() != 2:
			return _contract_error(source_name, "%s line must contain exactly two points" % prefix)
		...
		if not _positive_number(marking.get("widthMeters")):
			return _contract_error(source_name, "%s.widthMeters must be positive" % prefix)
		if not marking.get("color") is String or not marking.get("style") is String or \
				not marking.get("visibleInViewer") is bool:
			return _contract_error(source_name, "%s color/style/visibility types are invalid" % prefix)
	return ""
...
static func _validate_unique_ids(track: Dictionary, source_name: String) -> String:
	...
	for item in track["markings"]: ids.append(item["id"])
